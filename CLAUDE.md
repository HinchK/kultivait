...
uv run kultivait distill corpus --dry-run             # inspect anchors & held-out roster
uv run kultivait distill generate --live              # run dual-teacher synthetic generator
uv run kultivait distill train --base <base> --corpus-dir <dir> # train QLoRA adapter
uv run kultivait distill eval --model <m> --heldout <p> # held-out 5-gate eval
uv run kultivait distill export --base <b> --adapter-path <p> # fuse adapter & register in Ollama
uv run kultivait shadow                              # view shadow pass summary & cutover readiness
uv run kultivait cutover --model <distillate>        # flip live preprocessor seat (human confirmation)
...
4. **`server.py`** — FastAPI app exposing `/v1/chat/completions` (OpenAI-compatible), `/v1/messages` (Anthropic-compatible; tool support shipped — `tool_use` blocks, streaming and non-streaming), `/gate`, and `/harvest`. `_resolve_tier()` is the fallback rule: a tools-bearing request is **always** served by a local tool-capable tier even when classification points at a cloud tier, because cloud CLIs run their own agent loop and can't return client-side tool calls — the response's `kultivait.fallback_reason` reports this (`tools_unsupported` vs `no_backend` for virtual tiers). Every fallback also triggers an escalation save (see below). Server resolves the preprocessor model per request via `DistillSeat` and runs non-blocking, exception-isolated shadow passes on contested requests.
5. **`ledger.py`** — append-only JSONL at `~/.kultivait/ledger.jsonl`; every request is recorded with cost vs. a frontier-pricing baseline, so `kultivait harvest` can report cumulative savings. `record()` stores arbitrary `**extra` fields verbatim — routing metadata rides along with each entry rather than needing a schema migration. Preprocessed entries are tagged with `preprocess_model`.
6. **`escalations.py`** — when a request is force-downgraded to local (reported as `fallback_reason`) or otherwise, the full conversation is archived to `~/.kultivait/escalations/` *synchronously on the request path* but the paste-ready brief is only distilled lazily via `kultivait escalations --brief`, so the interactive request never pays the distillation cost.
...
8. **`distill/`** (`corpus.py`, `generator.py`, `trainer.py`, `eval.py`, `export.py`, `shadow.py`) — the model distillation & fine-tuning pipeline:
   - `corpus.py`: extracts real prompt anchors and isolates permanent held-out eval set from harvest.
   - `generator.py`: dual-teacher synthetic generation (`opencode` + `claude`) with agreement filtering and 3-stage validation.
   - `trainer.py`: Apple Silicon QLoRA via `mlx-lm` under strict hardware resource ladder.
   - `eval.py`: 5-gate validation on held-out data via production generate path.
   - `export.py`: fuses QLoRA weights into base and registers `kv-judge-<base>-g<gen>` with Ollama.
   - `shadow.py`: background post-response shadow pass on contested traffic; logs to `shadow.jsonl` and evaluates ADR 0017 cutover readiness.
...
- Ethos ordering when making product/behavior tradeoffs, per README: **reduce → right-size → localize** — prefer not sending a prompt at all, then the cheapest tier that can carry it, then local over cloud.
